...
from built_report_factory import build_weekly_report
...
def run_weekly_report_pipeline(excel_path: Path, context: ReportContext) -> BuiltWeeklyReport:
    raw_sheets = load_required_sheets(excel_path)
    raw_header_maps = validate_raw_sheets_structure(raw_sheets)

    normalized_kp_result = normalize_kp_rows(raw_sheets, raw_header_maps)
    normalized_delivery_result = normalize_delivery_rows(raw_sheets, raw_header_maps)

    kp_items = map_kp_rows(normalized_kp_result.rows)
    delivery_items = map_delivery_rows(normalized_delivery_result.rows)
    ...
    return build_weekly_report(
        ...
        critical_kp_items=sorted_kp_analysis.critical_overdue_items,
        warning_kp_items=sorted_kp_analysis.overdue_items,
        current_week_delivery_items=sorted_delivery_analysis.current_week_items,
        next_two_weeks_delivery_items=sorted_delivery_analysis.next_two_weeks_items,
        source_row_issues=[
            *normalized_kp_result.issues,
            *normalized_delivery_result.issues,
        ],
    )
